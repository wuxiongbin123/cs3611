...
void put(string key,string value,NodeInformation &nodeInfo){
	if(key == "" || value == ""){
        ...
        pair< pair<string,int> , lli > node = nodeInfo.findSuccessor(keyHash, false);
...
void get(string key,NodeInformation nodeInfo){
    ...
    if(key == ""){
        ...
        //在这里findSuccessor给的flag是true, 需要输出路由寻址过程
        pair< pair<string,int> , lli > node = nodeInfo.findSuccessor(keyHash, true);
        //给出key的哈希，找到key的后继，就是储存这个key的节点。
