...
cout << "Key is stored in the node with IP: " << self.first.first << ", Port: " << self.first.second << endl; 		
		...
		if(node.second == id){
            ...
            //这里设置了递归的目标，这里的node不在是本节点了。是目标节点。
			help.setServerDetails(serverToConnectTo,node.first.first,node.first.second);
...
pair< pair<string,int> , lli > NodeInformation::findSuccessor_routing(lli nodeId, bool printRouting, pair<pair<string, int>,lli > &serverNode){
    serverNode.second = -1;//这个用来决定是否有去找server。
    ...
    if(nodeId > id && nodeId <= successor.second){
        ...
        if(node.second == id){
            ...
            if(!printRouting){
                ...
                serverNode.first.first = node.first.first;
                serverNode.first.second = node.first.second;
                serverNode.second = node.second;
